# LumioGameEngine V3 (v1.1) 架构与开发规范

> **ArchitectureBaselineId**：`LGE-V1.1-2026-08-27`
...
> **v1.0 → v1.1 变更**：Server 架构门审整改——WorldSlotHost 聚合根与生命周期 epoch（ADR-001）、Runtime 见证的 FaultClass 故障分级（ADR-006）、Audit/Journal 分权与 correlation 作用域分层（ADR-011）、维护 deadline 时钟域与外部控制面/fencing（ADR-012）、§16 Server 子模块地图更新。字段级变更见各 Schema 与 Fixture。
